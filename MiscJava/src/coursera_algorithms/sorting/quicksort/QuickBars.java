package coursera_algorithms.sorting.quicksort;

import org.apache.commons.math3.stat.descriptive.rank.Median;

import coursera_algorithms.allClasses.stdlib.StdDraw;
...
public class QuickBars {
    ...
    private static void sort(double[] a, int lo, int hi) { 
        ...
        //System.out.println("Calculatin' the median between these elements: " + a[lo] + " " + a[lo +N/2] + " " + a[hi]);
        int m = median3(a, lo, lo + N/2, hi);
        double m22 = a[m];
        double m2 = new Median().evaluate(new double[]{a[lo], a[lo + N/2], a[hi]});
        exch(a, m, lo);
    ...
    public static void main(String[] args) {
        long M = 60000;//Integer.parseInt(args[0]);
        int N = 10000;//Integer.parseInt(args[1]);
        double[] a = new double[N];
        double[] b = new double[N];
        for (int i = 0; i < N; i++) {
            a[i] = ((int) (1 + Math.random() * M));
            b[i] = a[i];
        }
