...
public class QuickFindUF {
	...
	public void union(int p, int q) {
		if (connected(p, q))
			return;
		int root = id[p];
		int newElementInComponent = id[q];
		for (int i = 0; i < id.length; i++)
			if (id[i] == root)
				id[i] = newElementInComponent;
		count--;
	}
	...
	public static void main(String[] args) throws IOException {
		int[] array = MiscUtils.readUFArray();
		// int N = StdIn.readInt();
		QuickFindUF uf = new QuickFindUF(array.length);
		uf.id = array;
		System.out.println("Number of existing components: " + uf.count());
		MiscUtils.printBits(array, null);
		while (!StdIn.isEmpty()) {
			int p = StdIn.readInt();
			if (p == -1) {
				MiscUtils.printBits(array, null);
