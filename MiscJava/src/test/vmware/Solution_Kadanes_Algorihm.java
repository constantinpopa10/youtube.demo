...
public class Solution_Kadanes_Algorihm {
	...
	private static String FILE_2331142 = "input_2331142.txt";
	private static String FILE_1000000 = "input_1000000.txt";
	private static String FILE_5000000 = "input_5000000.txt";
	...
	public static void main(String args[]) throws Exception {
		...
		findMaxQuadratic(bits);
		
		findMaxLinear(bits);
		//testPrintAnsiColors();
	}

	private static void findMaxLinear(int[] bits) {
		System.out.println("==> O(n) algorithm result: " + getMaxSum(bits));
	}

	private static void findMaxQuadratic(int[] bits) {
		...
		System.out.println("Test duration quadractic : " + (end - start) / 1000d);
...
//		}
	...
	private static Map<Integer, Integer> getAllZeroIntervals(int[] bits) {			
		...
		//since we start taking into account 0s only after 1 was found, this initialization is needed for the first 0 found(assume the array starts with 0)
		...
		for (int k = 0; k < bits.length; k++) {
			if (bits[k] == 0) {
				...
				if (zeroes.get(currI) == null && !foundOne)
					zeroes.put(currI, k-1);
				foundOne=true;
			}						
		}
		return zeroes;
	...
	private static int getMaxSum(int[] bits) {		
		long start = System.currentTimeMillis();
		Integer currentSum = 0;
		Integer originalNrOf1 = countOnes(bits);		
	    java.util.List<Integer> maxes = new ArrayList<Integer>();		
		for(int i = 0; i< bits.length; i++){
			if(bits[i]==0){		
				if (currentSum>=0){
					currentSum++;
					maxes.add(currentSum);					
				}else{
					System.out.println("==> Skipped to another interval at index : " + i);
					currentSum=1;
				}			
			}else{
				currentSum--;
		...
		long end = System.currentTimeMillis();
		System.out.println("==> Test duration linear : " + (end - start) / 1000d);
		System.out.println("==> Number of 1s and 0s: " + bits.length);
		return originalNrOf1 + (!maxes.isEmpty() ? maxes.get(0) : 0);
	}
